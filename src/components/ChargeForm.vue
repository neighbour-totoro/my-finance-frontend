  ...
  <form @submit.prevent="true">
     ...
     <div style="margin-bottom:20px">
       ...
       <select class="form-select">
         <option v-for="row in revenue_acc"
         :key="row.id"
         :value="row.id">
           {{ row.name }}
         </option>
     ...
     <button class="btn btn-success" style="margin-right:5px">Добавить</button>
     <button class="btn btn-secondary" @click="this.$emit('close_window')">Закрыть</button>
  </form>
</div>
</template>
<script>
